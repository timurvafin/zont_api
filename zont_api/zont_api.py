...
class ZontAPI:
    ...
    def _make_request(self, method, endpoint, request=None):
        ...
        response = self.session.request(
            method, url, json=request, headers=headers, auth=auth
        )
    ...
    def _log_request(
        self, method, url, request_headers, request_body, response, response_json
    ):
    ...
    def __get_device_data(self):
        if (
            ...
            and (time.time() - self._device_cache.get("timestamp", 0)) < 60
        ):
